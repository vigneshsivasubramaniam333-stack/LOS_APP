...
import type { LoanPayuInitiatePayload } from '@/api/borrowerPortal'

export function BorrowerLoanPayuCheckoutPage() {
  const { loanId } = useParams<{ loanId: string }>()
  const location = useLocation()
  const formRef = useRef<HTMLFormElement>(null)
  const payu = (location.state as { payu?: LoanPayuInitiatePayload } | null)?.payu
  ...
  if (!payu || !loanId) {
    return <Navigate to={`/borrower/loans/${loanId ?? ''}/account`} replace />
  }
  ...
  return (
    <div className="py-16 text-center text-sm text-slate-500">
      ...
      <form ref={formRef} method="post" action={payu.baseUrl}>
        ...
        {payu.udf2 ? <input type="hidden" name="udf2" value={payu.udf2} /> : null}
