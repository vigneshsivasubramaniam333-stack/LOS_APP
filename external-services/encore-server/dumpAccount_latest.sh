...
[[ $# -lt 6 ]] && {
        echo Usage: `basename $0` 'userid password database tenant_code host include_referenced_entries [account_id1] [account_id2] ... > out_file_path' >&2
        exit 1
}
...
then
    echo "MYSQLDUMP could not be found" >&2
    exit 1
fi
...
if [ $INCLUDE_REFERENCED_ENTRIES == '0' ]
then
