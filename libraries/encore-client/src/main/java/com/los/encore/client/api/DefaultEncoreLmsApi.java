...
public class DefaultEncoreLmsApi implements EncoreLmsApi {
    ...
    public String openLoanAccount(EncoreOpenLoanParams request) {
        ...
        try {
            ...
            // TEMP FIX: hardcoded Encore product code - ignoring request.productCode()
            loanAccount.put("productCode", EncoreTemporaryOverrides.DEFAULT_ENCORE_PRODUCT_CODE);
            ...
            return "ENCORE-" + request.applicationNumber();
        } catch (Exception e) {
    ...
    public String findSummaryRaw(String accountId, boolean getAllDetails) {
        try {
            ...
            String transactionDetails = transport.httpGet(properties.getApi().getFindSummaries(), params);
            JsonNode arr = objectMapper.readTree(transactionDetails);
            if (arr.isArray() && arr.size() >= 1) {
                return arr.get(0).toString();
            }
            return transactionDetails;
        } catch (Exception e) {
            throw new RuntimeException("Encore findSummary failed: " + e.getMessage(), e);
        }
    }
}
