...
export function BorrowerIntakeWizard() {
  ...
  const selectedWorkflow = products.find((w) => w.loanProduct === form.loanProduct) ?? null
  const selectedWf = products.find((w) => w.loanProduct === form.loanProduct) ?? null
  const productLocked = Boolean(applicationId)

  useEffect(() => {
    if (!selectedWf || isInvoiceDiscountingProduct(form.loanProduct)) return
    setForm((f) => ({
      ...f,
      lmsProductCode: selectedWf.lmsProductCode?.trim() || DEFAULT_LMS_PRODUCT_CODE,
      lmsTenureUnit: selectedWf.lmsTenureUnit?.trim() || DEFAULT_LMS_TENURE_UNIT,
    }))
  }, [selectedWf?.id, selectedWf?.lmsProductCode, selectedWf?.lmsTenureUnit, form.loanProduct])
  ...
  async function goNext() {
    ...
    if (step === 1) {
      ...
      const v = validateBorrowerPersonalAddressStep(f0, selectedWorkflow)
      if (v) {
        setError(v)
        return
      }
    ...
    if (step === 2) {
      ...
      const v = validateBorrowerBankKycStep(f0, selectedWorkflow)
      if (v) {
        setError(v)
        return
      }
      ...
      {((step === 2 && !needColl) || (step === 3 && needColl)) && applicationId ? (
        <section className="space-y-4 bt-card p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {shouldShowKycIntakeField(selectedWorkflow, 'PAN_VERIFY', true) ? (
            <div className="sm:col-span-2">
              <span className="text-sm font-medium text-slate-800">PAN copy</span>
              <input
                type="file"
                accept=".pdf,image/*"
            ...
            <label className="block text-sm text-slate-700 sm:col-span-2">
              <span className="mb-1 block text-xs font-medium text-slate-500">Aadhaar (last 4 or full 12)</span>
              <input
                className="bt-input w-full"
                value={form.aadhaar}
